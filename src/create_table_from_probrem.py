...
def get_table_def(soup):
    ...
    for df in table_def_dfs:
        ...
        table_def_list.append(df.pivot(index='dummy', columns='列名', values='データ型').replace(
            type_dict).reset_index().drop(columns=['dummy']).to_dict(orient='records')[0])
...
if __name__ == "__main__":
    ...
    db_path = os.path.join(base_dir, "contests",
                           contest_name, problem_name + ".db")
    url = reduce(lambda x, y: parse.urljoin(x, y), [
                 base_url + "/", contest_name + "/", "problems/", problem_name])
